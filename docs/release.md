...
### 各 job 行为（单个 `.github/workflows/release.yml`）

一次推送 / `workflow_dispatch` 跑两个 job：`release-core` 然后 `release-mcp-server`（`needs: release-core`）。**不再**使用历史上的 `release-core.yml` / `release-mcp-server.yml`，也 **不再** 在 mcp-server job 里轮询 `npm view`。

**`release-core`**（tag 判据 `core-vX`）：

1. gate：远程已有 `core-vX` 则跳过 publish
2. pnpm install（`--frozen-lockfile`）
3. `pnpm publish`（`prepublishOnly`：typecheck + build，含 postbuild 修复 `node:sqlite`）
4. 打 `core-vX` 并推送

**`release-mcp-server`**（tag 判据 `mcp-server-vX`）：

1. 等 `release-core` 结束（core 未发或已存在都会让此 job 继续）
2. gate：远程已有 `mcp-server-vX` 则跳过
3. 构建 core（全新 checkout 无 `dist`）
...
- job 顺序保证 core 先发；**两者一起升级**时同时 bump 两个 `package.json` 的 version 再 push
...
- **刚发布完为什么通过 npmmirror/淘宝镜像安装还是找不到新版**？CI 发布走官方 registry。npmmirror 等公网镜像对官方 registry 的同步存在分钟级到小时级延迟，国内用户装到最新版需要等镜像同步，属于正常现象，稍后重试即可
