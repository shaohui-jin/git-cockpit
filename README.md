...
安装包只带窗壳，不含服务。启动时会先探活 `:3000`，未命中则找本机 Node.js 22+，再**试跑** `git-cockpit version` 确认服务可用；不可用（没装 / 依赖损坏）时，打包版会询问后自动执行 `npm i -g @shaohui_jin/git-cockpit-mcp-server@latest`，安装过程有日志窗口、可取消，全局目录不可写时自动改装到用户目录。已有服务时，打包版还会嗅探 npm 是否有更新版，确认后自动停服、升级并重载页面（有进行中任务时不升级）。开发态（`pnpm desktop:dev`）不自动安装/升级，缺失时提示先 `pnpm build`。
